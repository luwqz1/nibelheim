...
USER_AGENT: typing.Final = "CPython/{}.{} Nibelheim/{}".format(
...
DEFAULT_TCP_USER_TIMEOUT: typing.Final = (DEFAULT_TCP_KEEPALIVE_TIME + DEFAULT_TCP_KEEPALIVE_INTERVAL) * DEFAULT_TCP_KEEPALIVE_RETRIES
...
class RnetClient(ABCClient):
    ...
    async def request(
        ...
        **kwargs: typing.Unpack[Request],
        ...
        if (json_body := kwargs.pop("json", None)) is not None:
            kwargs["body"] = msgspec.json.encode(json_body)
    ...
    async def request_json(
        ...
        **kwargs: typing.Unpack[Request],
        ...
        return msgspec.json.decode(await self.request_bytes(url=url, method=method, data=data, **kwargs))
