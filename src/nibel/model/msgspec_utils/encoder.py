...
class Encoder:
    ...
    def __init__(self) -> None:
        ...
        self.enc_hooks = {
            ...
            fntypes.library.Variative: lambda s: s.v,
    ...
    def add_enc_hook[T](self, t: type[T], /) -> typing.Callable[[EncHook[T]], EncHook[T]]:
        def decorator(func: EncHook[T], /) -> EncHook[T]:
            encode_hook = self.enc_hooks.setdefault(typing.get_origin(t) or t, func)
    ...
    def add_abstract_enc_hook[T](self, abstract_type: type[T], /) -> typing.Callable[[EncHook[T]], EncHook[T]]:
        def decorator(func: EncHook[T], /) -> EncHook[T]:
            return self.abstract_enc_hooks.setdefault(typing.get_origin(abstract_type) or abstract_type, func)
    ...
    def enc_hook(self, obj: typing.Any, /) -> EncHook[typing.Any]:
        origin_type = typing.get_origin(obj) or obj
        ...
        if (enc_hook_func := self.enc_hooks.get(origin_type)) is None and (enc_hook_func := self.get_abstract_enc_hook(origin_type)) is None:
            raise NotImplementedError(
                f"Not implemented encode hook for object of type `{type_repr(origin_type)}`. You can implement encode hook for this object.",
            )

        return enc_hook_func(obj)
    ...
    def encode(
        ...
        *,
        ...
        buf = msgspec.json.encode(obj, enc_hook=self.enc_hook, order=order)
    ...
    def cast(self, obj: typing.Any, /) -> typing.Any:
        if (caster := self.cast_types.get(typing.get_origin(obj) or obj)) is not None:
